...
public abstract class Table<T extends PDPage> {
    ...
    public float draw() throws IOException {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            float groupedRowHeight = 0f;

            if (!row.isNoBreak()) {
                List<Row<T>> groupedRows = new ArrayList<>();

                for (int j = i + 1; j < rows.size(); j++) {
                    Row<T> nextRow = rows.get(j);
                    if (nextRow.isNoBreak()) {
                        groupedRows.add(nextRow);
                        groupedRowHeight += nextRow.getHeight();
                    } else {
                        break;
                    }
                }
                ...
                if (!groupedRows.isEmpty()) {
                    groupedRowHeight += row.getHeight();
                }
            }

            drawRow(row, groupedRowHeight);
        }
    ...
    private void drawRow(Row<T> row, float groupedRowHeight) throws IOException {
        ...
        if (!row.isNoBreak() && isEndOfPage(groupedRowHeight != 0 ? groupedRowHeight : rowHeight) && !header.contains(row)) {
